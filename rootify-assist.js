...
(function (raiz) {
  ...
  var MAPA = {
    ...
    recursos: { frase: N('Controle de cada app: ligue ou desligue funções (feature flags), ajuste comportamentos (remote config) e edite conteúdo (textos, fotos, vídeos). Depois, Publicar.', 'Control of each app: turn features on or off (feature flags), adjust behaviours (remote config) and edit content (texts, photos, videos). Then Publish.'),
      atalhos: [{ r: N('🏃 Aparelhos do RiseONE', '🏃 RiseONE equipment'), f: ir('conteudo-apps', 'rise-one'), perm: 'conteudo:ver' }, { r: N('🌐 Recursos globais', '🌐 Global features'), f: ir('recursos', '*', 'recursos') }, { r: N('🚀 Publicar', '🚀 Publish'), f: ir('publicar'), perm: 'publicar:ver' }],
    ...
    'conteudo-apps': { frase: N('Texto, foto e vídeos do YouTube de cada aparelho do RiseONE. Importe do app, edite e publique: o app lê sem commit.', 'Text, photo and YouTube videos of each RiseONE machine. Import from the app, edit and publish: the app reads it with no commit.'),
      atalhos: [{ r: N('🚀 Publicar', '🚀 Publish'), f: ir('publicar'), perm: 'publicar:ver' }],
      dica: N('Comece por "Importar do app": traz ids, nomes, textos e fotos que o app já tem.', 'Start with "Import from the app": it brings the ids, names, texts and photos the app already has.') },
